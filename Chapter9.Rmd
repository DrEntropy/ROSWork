...
For the intercept (after centering), I would consider a beta distribution with mean 0.75 and  and standard deviation about 0.4.   (then scale it by 100) This would ensure valid values. Since the mean of the beta distribution is $\frac{\alpha}{\alpha + \beta}$ , something like $\alpha = 3$ and $\beta=1$ is a good starting point:
...
Consider model $y=a + bx + \text{error}$ with $x\sim \text{Uniform}(-1,1})$ and independant priors on the coefficients and the residual error $\sigma$.  For a assume normal prior with mean 0 and standard deviation 1. For b assume normal porior with mean 0 and standard deviation 0.2
...
```{r}
ggplot(data = pivot_longer(coefdf |> select(Ns, lm_slope, stan_slope),-Ns), aes(x=Ns, y=value)) + geom_point(aes(color=name)) + scale_x_log10() 
```
