...
for(i in 1:folds)
{
  ...
  residuals_1[[i]] <- sum((predict(fit_1, newdata = test) - test$eval)^2)
  
  fit_3 <- stan_glm(eval ~ beauty + female + nonenglish , data = data, refresh=0)
  residuals_3[[i]] <- sum((predict(fit_3, newdata = test) - test$eval)^2)
}
```
...
```{r}
sqrt(5*sd(residuals_3)^2 + 5*sd(residuals_1)^2)
```