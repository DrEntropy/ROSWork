...
Note this requires tidy_ros to be cloned up one level

```{r }
beauty <- read_csv("../tidy_ros/Beauty/data/beauty.csv", col_types = "nnfiffff")
head(beauty)
```
...
```{r}
beauty |> summarize(total1 = sum((pred1- eval)^2),total3 = sum((pred3-eval)^2))
```

The two fits are not significantly different.  

```{r}
sds <- beauty |> group_by(fold) |> summarize(total1 = sum((pred1- eval)^2),total3 = sum((pred3-eval)^2)) |>
   ungroup() |> summarize(sd1 = sd(total1), sd3 = sd(total3))
sds

```

```{r}
sqrt(sds$sd1^2 + sds$sd3^2)*sqrt(5)
```
```{r}
ggplot(beauty |> mutate(rn = row_number(), err1= eval- pred1, err3 = eval - pred3) |>
         pivot_longer(c(err1,err3))) +
