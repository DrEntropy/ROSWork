...
Suppose $p$ = percentage who know the answer.  Then we would expect to see $100p$ correct answers from those who know the answers and, on average, $100(1-p)/4$ extra correct answers from the lucky guessers, so that $100*(p + (1-p)/4)=60$ correct answers.  With algebra we find $\hat{p} = 0.467 $.

The standard error of this estimate is $\sqrt{p(1-p)/n}= \sqrt{(1/4)(3/4)/((1-p)*100)} \approx 0.06$ 
